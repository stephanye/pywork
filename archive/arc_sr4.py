...
def main():
	...
	try:
		for pageindex in range(30):
			pageindex = pageindex+1
			deleteurl = "http://www.22.cn/ajax/yuming/guoqi.ashx?pageIndex="+str(pageindex)+"&pageCount=300&action=&position=1&doublep=0&digit=,1,2,3&suffix=,.com,.net&&seo=%20&deldate="+today+"&orderby=Length_a&fan"
			# deleteurl = "http://www.22.cn/ajax/yuming/guoqi.ashx?pageIndex="+str(pageindex)+"&pageCount=300&action=&position=1&doublep=0&digit=,1,2,3&suffix=,.com,.net&seo=%20&deldate="+today+"&orderby=Length_a&fan"
			jsonstr = urllib2.urlopen(deleteurl).read()
			data = json.loads(jsonstr)
			for d in data['data']:
	...
	avail = open('sr4.txt','ab+') # avail.csv为导出的结果文件，生成在当前脚本文件所在目录
	fields = ['----------------------------------------'+today+'----------------------------------------']
	w = csv.writer(avail)
	w.writerow(fields)
