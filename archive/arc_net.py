...
def main():
	...
	today = datetime.date.today()
	today = today.strftime("%Y-%m-%d")
	deletetotalurl = "http://www.22.cn/ajax/yuming/guoqi.ashx?pageIndex=1&pageCount=10&action=&position=1&doublep=0&digit=,1,2,3&suffix=,.net&seo=%20&deldate="+today+"&orderby=Length_a&fan"
	jsonstr = urllib2.urlopen(deletetotalurl).read()
	data = json.loads(jsonstr)
	totalcount=data['totalCount']
	total=totalcount//10000+1
	print 'total: %s' %total
	domains = []
	for pageindex in range(total):
		pageindex = pageindex+1
		deleteurl = "http://www.22.cn/ajax/yuming/guoqi.ashx?pageIndex="+str(pageindex)+"&pageCount=10000&action=&position=1&doublep=0&digit=,1,2,3&suffix=,.net&seo=%20&deldate="+today+"&orderby=Length_a&fan"
		jsonstr = urllib2.urlopen(deleteurl).read()
		data = json.loads(jsonstr)
		for d in data['data']:
			print d['Domain']
	...
	avail = open('out_net.txt','ab+') # avail.csv为导出的结果文件，生成在当前脚本文件所在目录
	fields = ['----------------------------------------'+today+'----------------------------------------']
	w = csv.writer(avail)
	w.writerow(fields)
